...
def guess_role(text):
    ...
    for r in CFG()["roles"]:
        ...
        if s > score: best, score = r["name"], s
...
def run_skill_check(text):
    ...
    # enough evidence: return N/A (None)
    if not enough:
        rs = []
        if len(found) < 4:
        ...
        return {
            ...
            "skills_found": found,
            "mismatch_score": None,   # N/A
            "off_role_skills": [],
            "flag": False,
            "reasons": rs
        }

    # Enough evidence: mismatch score
...
def _k(x): return int(float(x[:-1])*1000) if x.lower().endswith("k") else int(float(x))
...
def parse_salary_inr_month(text):
    ...
    m = re.search(r"\b(\d+(?:\.\d+)?k?)\s*(to|\-|–)\s*(\d+(?:\.\d+)?k?)\b", t)
    if m and month_hint:
        a,b = _k(m.group(1)), _k(m.group(3))
        return {"ok": True, "min": min(a,b), "max": max(a,b), "confidence": "HIGH"}

    m = re.search(r"\b(stipend\s*[:\-]?\s*)?(\d+(?:\.\d+)?k?)\s*(per month|monthly|month|pm)\b", t)
    if m:
        v = _k(m.group(2))
    ...
    if m:
        ...
        return {"ok": True, "min": int(min(lo,hi)*100000/12), "max": int(max(lo,hi)*100000/12), "confidence": "HIGH"}
...
def run_salary_check(text, role_guess, role_conf):
    ...
    if not p["ok"]:
        return {"salary_parsed": False, "zone": "NO_SALARY", "anomaly_score": None, "flag": False,
    ...
    if mid <= mx:
        zone,score,label,theme = "GREEN", 0.10 + 0.25*max(0,min(1,(mid-mn)/max(1,(mx-mn)))), "Within market range", "SAFE"
    elif mid <= hi:
        zone,score,label,theme = "YELLOW", 0.35 + 0.35*max(0,min(1,(mid-mx)/max(1,(hi-mx)))), "High-end (Competitive)", "HOT"
    else:
        zone,score,label,theme = "RED", 0.70 + min(0.30, math.log(mid/max(1,hi)+1,10)/2), "Too Good to be True (Scam Territory)", "DANGER"

    score = float(max(0,min(1,score)))
    flag = bool(zone=="RED" and role_conf>=0.60 and p["confidence"]=="HIGH")

    rs = []
    if role_conf < 0.60: rs.append("Role confidence is low, so salary anomaly detection is conservative.")
    if p["confidence"] != "HIGH": rs.append("Salary parse confidence is not high; treating anomaly cautiously.")
    rs.append(("Salary looks normal" if zone=="GREEN" else "Salary looks competitive" if zone=="YELLOW" else "Salary is above typical high-end") + f" for {role_guess}.")

    return {"salary_parsed": True, "currency": "INR", "pay_basis": "MONTH_EQUIV",
            "offered_min": off_min, "offered_max": off_max, "offered_mid": mid,
            "market_min": mn, "market_max": mx, "high_end_max": hi,
            "zone": zone, "anomaly_score": round(score,3), "flag": flag, "reasons": rs,
